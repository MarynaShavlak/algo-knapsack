...
# `core.knapsack_dp`; max(...) розгорнуто у take/skip, як у журнальній версії
# конспекту, щоб гілки «взяти»/«не брати» підсвічувались окремими рядками.
# ---------------------------------------------------------------------------
CODE: List[str] = [
    "K = [[0]*(W+1) for i in range(n+1)]",            # 0  init: таблиця з нулів
    "",                                               # 1
    ...
    "            take = val[i-1] + K[i-1][w - wt[i-1]]",  # 7  ← варіант «взяти»
