...
def knapsack_brute_force(
    ...
    Про нічиї: якщо оптимальних наборів **кілька**, повертається перший знайдений
    у порядку перебору (менші підмножини — раніше). Він може відрізнятися від
    набору, який відновлює :func:`reconstruct_items` (там нічия трактується як
    «не брати»), — але **вартість завжди однакова**, і саме її звіряють тести.
...
def knapsack_greedy(items: "list[Item]", capacity: int) -> int:
    ...
    Передумова жадібного підходу — додатні ваги: ``ratio`` ділить вартість на
    вагу (її перевіряє інструментована :func:`greedy_steps`).
...
def greedy_steps(
    ...
    :raises ValueError: якщо вага якогось предмета не додатна — питома цінність
        ``value / weight`` для такого предмета не визначена (та сама умова
        неявно закладена і в конспектний :class:`Item`).
    """
    for k, (weight, _) in enumerate(items):
        if weight <= 0:
            raise ValueError(
                f"greedy_steps: вага предмета №{k} має бути ≥ 1 (отримано "
                f"{weight}) — питома цінність ділить вартість на вагу.")
...
def reconstruct_items(K: Table, wt: Sequence[int], W: int) -> List[int]:
    ...
    складається. Відновлення йде з правого нижнього кута вгору — тим самим ходом,
    що й фрагмент у конспекті: якщо ``K[i][w] != K[i-1][w]``, значення в рядку
    ``i`` могло взятися лише з гілки «взяти» — отже, предмет ``i`` у наборі, і ми
    «звільняємо» його вагу. На нічиїх (``K[i][w] == K[i-1][w]``, хоч «взяти» дало
    б стільки само) предмет **не** береться.

    Відмінність від конспектного фрагмента: той складає список **імен**
    (``names[i - 1]``), а ця функція повертає **0-базові індекси** — імена до
    них додають уже приклади (через ``item_names``).
