...
Репозиторій — навчальний матеріал: чисті реалізації з конспекту + докладні візуалізації кожного кроку. Увесь розбір нижче відтворюється кодом із [`examples/`](examples), а рисунки лежать у [`docs/images/`](docs/images). Першоджерело — ноутбук-конспект [`knapSack.ipynb`](knapSack.ipynb) у корені репозиторію.
...
> У конспекті таблиця в цьому фрагменті називається `dp` — це та сама `K` (обидві назви трапляються в літературі).
